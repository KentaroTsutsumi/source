program ksf_bin2txt
  ...
  implicit none
  !
  integer :: fi = 10, ng(3), nk, nb, nmf, imf
  real(8) :: qv(3)
  real(8),allocatable :: mf(:), wmf(:), Ksf(:,:,:,:)
  ...
  do iq = iniq, lasq
    !
    write(fname,*) iq
    write(fname1,*) "vc", trim(adjustl(fname)), "_1.dat"
    write(fname2,*) "vc", trim(adjustl(fname)), "_1.txt"
    write(fname3,*) "Ksf", trim(adjustl(fname)), "_1.dat"
    write(fname4,*) "Ksf", trim(adjustl(fname)), "_1.txt"
  ...
  write(*,*) nmf
    !
    nk = product(ng(1:3))
    allocate(mf(nmf), wmf(nmf), Ksf(0:nmf,nb,nb,nk))
    !
    do imf = 1, nmf
